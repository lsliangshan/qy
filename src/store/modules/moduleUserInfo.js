...
const moduleUserInfo = {
  ...
  actions: {
    cache ({ state, rootState }, data) {
      rootState.loginInfo = JSON.parse(JSON.stringify(data))
      let loginInfoStr = JSON.stringify(data)
      let cryptoText = CryptoJS[rootState.cryptoType].encrypt(loginInfoStr, rootState.privateKey).toString()
      utils.storage.setItem(rootState.localStorageKeys.userInfo, cryptoText)
    },
