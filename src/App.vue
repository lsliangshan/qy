<template>
  ...
  <f7-app :params="f7params"
          ...
          :class="'color-theme-' + (activeThemeColor ? activeThemeColor.value : defaultColorTheme.value)">
    ...
    methods: {
      ...
      getUserSettings () {
        return new Promise(async resolve => {
          ...
          let osTheme = await StorageUtil.getItem(this.localStorageKeys.osTheme)
          if (!osTheme || (['ios', 'md'].indexOf(osTheme) < 0)) {
            osTheme = 'ios'
          }
          this.store.commit(types.SET_OS_THEME, {
